...
train_loader, dev_loader, test_loader, lang = create_dataloaders( args.batch )

out_slot = len(lang.id2slot)
out_int = len(lang.id2slot)
...
model = ModelIAS( args.hidden, out_slot, out_int, args.emb, vocab_len, pad_index=PAD_TOKEN, n_layer=args.layers, dropout = args.do, bidirectional=args.bidirect ).to( device ) 
